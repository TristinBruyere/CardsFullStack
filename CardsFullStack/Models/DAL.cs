...
namespace CardsFullStack.Models
{
    ...
    //
    //  IMPORTANT!!!
    //      Some of the Http stuff was moved to a NuGet package!!
    //      The package is called "Microsoft.AspNet.WebApi.Client"
    ...
    public class DAL
    {
        ...
        public static async Task<IEnumerable<Card>> InitializeDeck()
        {
            ...
            // Step 2: Save the deck into the DB (we have a function that does that below)
            Deck mydeck = saveDeck(deckresp.deck_id, "User1");

            // Step 3: Call the API to get two cards for that deck
            response = await client.GetAsync($"https://deckofcardsapi.com/api/deck/{deckresp.deck_id}/draw/?count=2");
            ...
            {
                saveCard(mydeck.deck_id, cardresp.image, cardresp.code, "User1");
            }
        ...
        // Get More Cards
        //  Draw two cards (from whick deck? -- this will be a parameter)
        //  Save those cards in our own DB
        //  Return those cards
        ...
        //          In other words, the code velow will not use DeckResponse or CardResponse
        ...
        {
            // To make this work we have to alse add in the using statement for Dapper itself.
